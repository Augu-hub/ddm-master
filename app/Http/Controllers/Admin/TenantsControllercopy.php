...
class TenantsControllercopy extends Controller
{
   ...
   private function provisionTenant(Tenant $tenant): void
{
    ...
    // 3) MIGRATIONS de base du tenant (si tu as d'autres tables communes)
    $this->runTenantMigrations($tenant->db_name, [
        'database/migrations/tenant', // facultatif si tu as un noyau commun
    ]);

    // 4) MIGRATIONS du MODULE PROCESS (anglais)
    $this->runTenantMigrations($tenant->db_name, [
        'database/migrations/tenant/process',   // ← on utilise "process" (EN)
        // 'database/migrations/tenant/processus', // (si tu conserves un dossier FR, laisse-le ici)
    ]);

    // 5) Vérifie/complète le module process si besoin (diagnostic)
...
{
    throw new \RuntimeException('MySQL ne supporte pas RENAME DATABASE. Utilise une procédure export/import ou copie table par table.');
}
    ...
    /**
 * Exécute une liste de chemins de migrations sur la connexion tenant.
 * Ignore automatiquement les dossiers qui n’existent pas.
 */
private function runTenantMigrations(string $dbName, array $paths): void
{
    foreach ($paths as $path) {
        if (! is_dir(base_path($path))) {
            \Log::warning("Chemin de migration introuvable pour {$dbName}: {$path}");
            continue;
        }
        ...
        \Log::info("Migrations appliquées ({$path}) sur {$dbName}", [
...
/**
 * Diagnostique la présence des tables du module process
 * et logge ce qui manque (utile pour audit/support).
 */
private function ensureProcessModule(Tenant $tenant): void
{
    $conn = Schema::connection('tenant');
    $expected = [
        // tronc (peut déjà exister via tes migrations envoyées)
        'processes', 'activities',

        // annexes module process (EN)
        'process_diagrams', 'process_evaluations', 'process_kpis', 'process_links',
        'activity_steps', 'activity_flows', 'activity_controls', 'activity_raci', 'activity_idea',
    ];

    $missing = [];
    foreach ($expected as $tbl) {
        if (! $conn->hasTable($tbl)) $missing[] = $tbl;
    }

    if ($missing) {
        \Log::warning("Tables process manquantes sur {$tenant->db_name}", $missing);
        // Si besoin : relancer la migration du dossier process (sécurisé par table migrations)
        Artisan::call('migrate', [
            '--database' => 'tenant',
            '--path'     => 'database/migrations/tenant/process',
            '--force'    => true,
        ]);
    } else {
