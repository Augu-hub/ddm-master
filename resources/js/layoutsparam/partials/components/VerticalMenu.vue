...
function detectModuleFromUrl(): string | null {
...
async function fetchMenuStructure(moduleCode?: string | null): Promise<ServerMenuNode[]> {
    // Toujours envoyer le module : props → URL → rien
    const resolvedCode = moduleCode ?? detectModuleFromUrl()
    const qs = resolvedCode ? `?module=${encodeURIComponent(resolvedCode)}` : ''
