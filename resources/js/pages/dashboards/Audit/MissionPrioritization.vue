          ...
          <table class="mission-table">
              ...
              <tr>
                ...
                <th v-for="factor in store.allFactors" :key="`th-${factor.id}`" class="factor-col">
                  <div class="factor-header">
                    ...
                    <div class="factor-sublabel">Moy: {{ getFactorAverageAcrossAll(factor.id).toFixed(2) }}</div>
                  </div>
                </th>
            ...
            <tbody>
              <tr v-for="mission in entityGroup.missions" :key="`row-${mission.id}`" class="mission-row">
                ...
                <!-- FACTEURS AVEC ANCIEN SCORE ET COULEUR -->
                <td v-for="factor in store.allFactors" :key="`cell-${mission.id}-${factor.id}`" class="score-cell">
                  ...
                  <div v-if="displayOldScore(mission.id, factor.id)" class="old-score-badge" :style="{ borderColor: getOldScoreColor(mission.id, factor.id), backgroundColor: getOldScoreColor(mission.id, factor.id) + '30' }">
                    {{ getOldScore(mission.id, factor.id) }}
                  </div>

                  <!-- BOUTONS: COULEUR SEULEMENT SI ACTIF, SINON GRIS AVEC ANCIEN SCORE EN DASHED -->
                  <div class="score-buttons">
                    <button
                      ...
                      :style="getButtonStyle(mission.id, factor.id, scale)"
      ...
      <div class="legend-box">
        ...
        <div class="legend-items" style="margin-top: 1rem;">
          ...
          <div class="legend-item">
            ...
            <span>🎨 Badge coloré en haut à droite de chaque cellule + Bouton en dashed semi-transparent</span>
          </div>
          <div class="legend-item">
            <strong>Bouton Actif:</strong>
            <span>Couleur PLEINE avec bordure visible</span>
          </div>
          <div class="legend-item">
            <strong>Bouton Inactif:</strong>
            <span>Gris clair #f0f0f0</span>
          </div>
        </div>
      </div>
    </div>
              ...
              <div class="table-responsive">
                <table class="table table-sm table-bordered">
                  <thead class="table-light">
                    <tr>
                      <th>Facteur</th>
                      <th class="text-center" style="width: 100px">Score Actuel</th>
                      <th class="text-center" style="width: 100px">Ancien Score</th>
                      <th class="text-center" style="width: 120px">Changement</th>
                      <th>Échelle</th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="factor in store.allFactors" :key="factor.id">
                      ...
                      <td class="text-center">
                        <span v-if="getOldScore(selectedMission.id, factor.id)" 
                              class="badge" 
                              :style="{ backgroundColor: getOldScoreColor(selectedMission.id, factor.id) + 'cc' }">
                          {{ getOldScore(selectedMission.id, factor.id) }}
                        </span>
                        <span v-else class="text-muted">—</span>
                      </td>
                      <td class="text-center">
                        <span v-if="getOldScore(selectedMission.id, factor.id) && selectedMission.factorScores[factor.id]" 
                              :class="['badge', getChangeClass(selectedMission.factorScores[factor.id], getOldScore(selectedMission.id, factor.id))]">
                          {{ getChangeLabel(selectedMission.factorScores[factor.id], getOldScore(selectedMission.id, factor.id)) }}
                        </span>
                        <span v-else class="text-muted">—</span>
                      </td>
                      <td>
...
// ✅ CALCULER LA MOYENNE D'UN FACTEUR POUR TOUTES LES MISSIONS
const getFactorAverageAcrossAll = (factorId: number): number => {
  const scores = missionsByYear.value
    .map(m => store.getScore(m.id, factorId))
    .filter(s => s > 0)
...
// ✅ ANCIEN SCORE - OBTENIR LA VALEUR
...
// ✅ AFFICHER SI ANCIEN SCORE EXISTE
const displayOldScore = (missionId: number, factorId: number): boolean => {
  return getOldScore(missionId, factorId) > 0
}

// ✅ COULEUR POUR L'ANCIEN SCORE
const getOldScoreColor = (missionId: number, factorId: number): string => {
  const score = getOldScore(missionId, factorId)
  const scale = factorScales.value.find(s => s.value === score)
  return scale ? scale.color : '#999'
}
...
// ✅ AFFICHER COULEUR SEULEMENT SI ACTIF
const getButtonStyle = (missionId: number, factorId: number, scale: FactorScale) => {
  const isActive = store.getScore(missionId, factorId) === scale.value
...
// ✅ COMPARAISON ANCIEN VS NOUVEAU SCORE (POUR MODAL)
const getChangeLabel = (current: number, old: number): string => {
  if (current > old) return `+${(current - old).toFixed(1)} ↑`
  if (current < old) return `${(current - old).toFixed(1)} ↓`
  return 'Identique'
}

const getChangeClass = (current: number, old: number): string => {
  if (current > old) return 'bg-success'
  if (current < old) return 'bg-danger'
  return 'bg-secondary'
}
...
/* ✅ ANCIEN SCORE - PETIT BADGE COLORÉ EN HAUT À DROITE */
.old-score-badge {
  position: absolute;
  top: 2px;
  right: 3px;
...
/* ✅ BOUTON ACTIF: COULEUR PLEINE + BORDURE VISIBLE */
.btn-scale.active {
  opacity: 1 !important;
...
/* ✅ BOUTON INACTIF: GRIS CLAIR + BORDURE LÉGÈRE */
.btn-scale:not(.active) {
...
/* ✅ ANCIEN BOUTON SÉLECTIONNÉ: SEMI-TRANSPARENT + DASHED */
