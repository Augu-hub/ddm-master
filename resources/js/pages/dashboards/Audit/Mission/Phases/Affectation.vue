    ...
    <div class="paf-shell">
      ...
      <div class="paf-bar">
        ...
        <div v-if="step >= 2 && localMission?.id" class="paf-mission-info">
          ...
          <span class="mi-badge" v-if="localMission.audit_type_label">
            {{ localMission.audit_type_label }}
          </span>
          <span v-if="step === 3" class="mi-count">
            <i class="ti ti-list-check"></i> {{ checkedPhaseIds.size }} phase(s)
          </span>
        </div>

        <div class="paf-bar-right">
          <!-- Indicateur chargement -->
          <span v-if="loadingData" class="loading-pill">
      ...
      <div v-if="step === 1" class="paf-content">
        ...
        <div class="s1-table-wrap">
          <table class="g-table">
            ...
            <tbody>
              ...
              >
                <td>
                  <code class="mono blue">{{ m.code_mission }}</code>
                </td>
                <td class="ell max240">{{ m.libelle }}</td>
                <td>
                  <span class="chip cb" v-if="m.type_label || m.audit_type_label">
                    {{ m.type_label || m.audit_type_label }}
                  </span>
                ...
                <td>
                  <div class="prog-bar-wrap" v-if="m.total_aff > 0" :title="`${m.completed_aff}/${m.total_aff}`">
                    <div class="prog-bar">
                      <div class="prog-fill" :style="{ width: m.pct_aff + '%' }"></div>
                    </div>
                ...
                <td>
                  <span class="chip" :class="stChip(m.status)">{{ stLbl(m.status) }}</span>
                </td>
                <td>
                  <a :href="pageUrl(m.id)" class="row-btn" title="Charger" @click.stop>
        ...
        <div v-if="selectedMission" class="s1-footer">
          ...
          <a :href="pageUrl(selectedMission.id)" class="act act-primary">
            Charger cette mission <i class="ti ti-arrow-right"></i>
          </a>
        </div>
      </div>
      ...
      <div v-if="step === 2" class="paf-content s2-layout">
        ...
        <div class="s2-side">
          ...
          <!-- Formulaires chargés (depuis session / ddmparam) -->
          <div class="side-block" v-if="formsList.length > 0">
            <label class="side-label">
              Formulaires chargés
              <span class="badge-count">{{ formsList.length }}</span>
            </label>
            <div class="forms-mini-list">
              <div
                v-for="f in formsList.slice(0, 8)" :key="f.code"
                class="form-mini-item"
                :class="{ 'fmi-child': f.level > 1 }"
              >
                <i :class="f.icon || 'ti ti-file'" class="fmi-icon"></i>
                <span class="fmi-code">{{ f.code }}</span>
                <span class="fmi-label">{{ f.label }}</span>
              </div>
              <div v-if="formsList.length > 8" class="form-mini-more">
                +{{ formsList.length - 8 }} autres…
              </div>
            </div>
            <div class="forms-source-badge">
          ...
          <div class="side-block" v-else-if="!loadingData">
            ...
            <div class="no-forms-hint">
              <i class="ti ti-alert-triangle amber"></i>
              Aucun formulaire trouvé pour ce type d'audit.
            </div>
          </div>

          <!-- Compteur phases sélectionnées -->
          <div class="side-block">
          ...
          <div class="side-btns">
            <button class="act act-ghost sm" @click="checkAllPhases">
              <i class="ti ti-checks"></i> Tout
            </button>
            <button class="act act-ghost sm" @click="uncheckAllPhases">
        ...
        <div class="s2-groups-wrap">
          <div v-if="loadingData" class="loading-phases">
            <span class="spin"></span> Chargement des phases…
          </div>
            ...
            <div class="s2-groups">
              <div v-for="group in filteredPhaseGroups" :key="group.phase_type" class="pg-card">
                ...
                >
                  ...
                  <span class="pg-name" :style="'color:' + ptColor(group.phase_type)">
                    {{ ptLabel(group.phase_type) }}
                  </span>
                ...
                <div v-if="openGroups.has(group.phase_type)" class="pg-body">
                  <template v-for="p in group.phases" :key="p.id">
                    ...
                    <label
                      ...
                      :key="c.id"
                      class="ph-row ph-child"
                      :class="{ mandatory: c.is_mandatory }"
                    >
                      ...
                      <span v-if="c.form_code && forms[c.form_code]" class="ph-form-badge" :title="forms[c.form_code].label">
                        <i :class="forms[c.form_code].icon || 'ti ti-file'"></i>
                        {{ c.form_code }}
                      </span>
      ...
      <!-- ════════════════════════════════════════════════════════
           ÉTAPE 3 — Tableau d'affectation par entité
      ════════════════════════════════════════════════════════ -->
      <div v-if="step === 3" class="paf-content s3-layout">
        ...
        <template v-if="activeEntityId">
          ...
          <div class="ent-bar">
            ...
            <span v-if="entityDateErrors(activeEntityId).length" class="err-pill">
              <i class="ti ti-alert-triangle"></i> {{ entityDateErrors(activeEntityId).length }} erreur(s)
            </span>
            <span v-if="warnings.length" class="warn-pill" @click="showWarnings = !showWarnings">
              <i class="ti ti-info-circle"></i> {{ warnings.length }} avertissement(s)
            </span>
            <div class="ent-bar-right">
              <button class="act act-ghost sm" @click="cascadeEntity(activeEntityId)" title="Aligner les dates sur les précédentes">
                <i class="ti ti-sort-ascending"></i> Cascade
              </button>
              <button class="act act-ghost sm" @click="selectAllAuds(activeEntityId)" title="Cocher tous les auditeurs sur toutes les phases">
                <i class="ti ti-user-check"></i> Auditeurs
              </button>
              <button class="act act-danger-ghost sm" @click="clearEntity(activeEntityId)" title="Réinitialiser cette entité">
                <i class="ti ti-eraser"></i>
              </button>
            </div>
          </div>

          <!-- Panneau avertissements (collapsible) -->
          <div v-if="showWarnings && warnings.length" class="warnings-panel">
            <div class="wp-head">
              <i class="ti ti-info-circle"></i> Avertissements de la dernière sauvegarde
              <button @click="showWarnings = false" class="wp-close">✕</button>
            </div>
            <ul class="wp-list">
              ...
              <tbody>
                <template v-for="grp in checkedByGroup" :key="'g' + grp.phase_type">
                  ...
                  <template v-for="ph in grp.phases" :key="'p' + ph.id">
                    <!-- Parent verrouillé (a des sous-phases sélectionnées) -->
                    <tr v-if="ph.hasSelectedChildren" class="tr-locked">
                      ...
                      <td :colspan="8 + entityAuds(activeEntityId).length" class="locked-lbl">
                        {{ ph.label }}
                    ...
                    <tr
                      ...
                      :class="{
                        ...
                        'tr-warn': hasStartErr(ph.id, activeEntityId),
                      }"
                    ...
                    >
                      ...
                      <td class="td-form">
                        <template v-if="ph.form_code && forms[ph.form_code]">
                          <a
                            ...
                            :href="buildFormUrl(forms[ph.form_code].url_path, localMission.id)"
                            target="_blank"
                            class="form-link"
                            :title="forms[ph.form_code].label"
                          >
                            <i :class="forms[ph.form_code].icon || 'ti ti-file'"></i>
                            <span>{{ ph.form_code }}</span>
                          </a>
                      ...
                      <td class="td-status">
                        <select
                          class="status-sel"
                          :value="getCfg(ph.id, activeEntityId).status || 'pending'"
                          :disabled="!isCfgChk(ph.id, activeEntityId)"
                          @change="setCfg(ph.id, activeEntityId, { status: $event.target.value })"
                        >
                ...
                <tr v-if="!checkedByGroup.length">
                  <td :colspan="10 + entityAuds(activeEntityId).length" class="empty-row">
                    Aucune phase sélectionnée — retournez à l'étape 2
                  </td>
                </tr>
                ...
                <tr v-if="checkedByGroup.length && activeEntityId" class="tr-total">
                  <td colspan="7">
                    <span class="total-lbl"><i class="ti ti-sum"></i> TOTAL</span>
                  </td>
                  ...
                  <td :colspan="entityAuds(activeEntityId).length + 1" class="total-hint muted small">
                    union des intervalles (sans répétitions)
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div v-else class="no-ent">
          ...
          Choisissez une entité ci-dessus
        </div>
      </div>
...
const props = defineProps({
  ...
  forms:       { type: Object, default: () => ({}) },  // {code: {label,url_path,icon,phase_num,...}}
...
const filteredMissions = computed(() => {
  ...
  return (props.allMissions as any[]).filter(m => {
    const mq = !q || [m.code_mission, m.libelle, m.entities_list].some((s: any) =>
...
const pct = computed(() =>
  totalPhases.value ? Math.round(checkedPhaseIds.value.size / totalPhases.value * 100) : 0
)

const activeEntity = computed(() =>
  localEntities.value.find(e => e.id === activeEntityId.value) || null
)

// Liste des formulaires (depuis forms prop)
...
// Phases filtrées par recherche texte
const filteredPhaseGroups = computed(() => {
  const q = phaseSearch.value.trim().toLowerCase()
  if (!q) return localPhases.value

  return localPhases.value.map(g => ({
    ...g,
    phases: g.phases
      .map((p: any) => ({
        ...p,
        children: (p.children || []).filter((c: any) =>
          c.label.toLowerCase().includes(q) || (c.code_full || c.code).toLowerCase().includes(q)
        )
      }))
      .filter((p: any) =>
        p.label.toLowerCase().includes(q) ||
        (p.code_full || p.code).toLowerCase().includes(q) ||
        p.children.length > 0
      )
  })).filter(g => g.phases.length > 0)
})

// Phases cochées organisées par groupe (pour le tableau étape 3)
...
const allChkPhases = computed(() =>
  checkedByGroup.value.flatMap(g => g.phases.filter((p: any) => !p.hasSelectedChildren))
)

// ══════════════════════════════════════════════════════════════════
//  INIT — Charger assignments existants depuis props
// ══════════════════════════════════════════════════════════════════
...
for (const [k, v] of Object.entries(props.assignments || {})) {
  ...
  entCfg[p][e] = {
    checked:       true,
    status:        vv.status        || 'pending',
    planned_start: vv.planned_start || null,
    planned_end:   vv.planned_end   || null,
    notes:         vv.notes         || null,
    auditeurs:     audIds,
  }
}
...
onMounted(() => {
  ...
  if (step.value === 3) {
    ...
    if (!activeEntityId.value && localEntities.value.length)
      activeEntityId.value = localEntities.value[0].id
  }

  // Déterminer la source des formulaires
...
watch(() => props.entities, v => {
  ...
  if (!activeEntityId.value && (v as any[]).length)
    activeEntityId.value = (v as any[])[0].id
}, { deep: true })
...
watch(() => props.phases, v => {
  ...
  openGroups.value = new Set((v as any[]).map((g: any) => g.phase_type))
}, { deep: true })

watch(() => props.mission, v => {
  localMission.value = v ? { ...v } : {}
  if (v) step.value = 2
})

watch(() => props.forms, v => {
  forms.value = (v as any) || {}
}, { deep: true })
...
function pageUrl(id: number) {
  return window.location.pathname.split('?')[0] + '?mission_id=' + id
}
...
function toggleGroupCheck(g: any, checked: boolean) {
  const s = new Set(checkedPhaseIds.value)
  for (const p of g.phases) {
    if (!p.is_mandatory || checked) {
      if (checked) { s.add(p.id); initPh(p.id) } else s.delete(p.id)
    }
    for (const c of (p.children || [])) {
      if (!c.is_mandatory || checked) {
        if (checked) { s.add(c.id); initPh(c.id) } else s.delete(c.id)
      }
    }
  }
  checkedPhaseIds.value = s
...
function checkAllPhases() {
  ...
  for (const g of localPhases.value)
    for (const p of g.phases) {
      s.add(p.id); initPh(p.id)
      for (const c of (p.children || [])) { s.add(c.id); initPh(c.id) }
    }
  checkedPhaseIds.value = s
}
...
function uncheckAllPhases() {
  ...
  for (const g of localPhases.value)
    for (const p of g.phases) {
      if (p.is_mandatory) s.add(p.id)
      for (const c of (p.children || [])) if (c.is_mandatory) s.add(c.id)
    }
  checkedPhaseIds.value = s
}

function toggleGrp(pt: string) {
  const s = new Set(openGroups.value)
  s.has(pt) ? s.delete(pt) : s.add(pt)
  openGroups.value = s
}

function grpAllChk(g: any) {
  return g.phases.every((p: any) =>
    checkedPhaseIds.value.has(p.id) &&
    (p.children || []).every((c: any) => checkedPhaseIds.value.has(c.id))
  )
}
function grpPartialChk(g: any) {
  return !grpAllChk(g) && g.phases.some((p: any) =>
    checkedPhaseIds.value.has(p.id) ||
    (p.children || []).some((c: any) => checkedPhaseIds.value.has(c.id))
  )
}
function cntChkInGrp(g: any) {
  let n = 0
  for (const p of g.phases) {
    if (checkedPhaseIds.value.has(p.id)) n++
    for (const c of (p.children || [])) if (checkedPhaseIds.value.has(c.id)) n++
  }
  return n
}
function cntGrp(g: any) {
  let n = 0
  for (const p of g.phases) { n++; n += p.children?.length || 0 }
  return n
}
...
function forceMandatory() {
  for (const g of localPhases.value) for (const p of g.phases) {
    const force = (ph: any) => {
      ...
      initPh(ph.id)
      for (const e of localEntities.value) {
        if (!entCfg[ph.id][e.id])
          entCfg[ph.id][e.id] = { checked: true, status: 'pending', planned_start: null, planned_end: null, notes: null, auditeurs: [] }
        else entCfg[ph.id][e.id].checked = true
      }
    }
    force(p)
    for (const c of (p.children || [])) force(c)
  }
}
...
//  CONFIG ENTITÉ
// ══════════════════════════════════════════════════════════════════
function getCfg(pid: number, eid: number) {
  return entCfg[pid]?.[eid] || { checked: false, status: 'pending', planned_start: null, planned_end: null, notes: null, auditeurs: [] }
}

function setCfg(pid: number, eid: number, patch: any) {
  initPh(pid)
  if (!entCfg[pid][eid])
    entCfg[pid][eid] = { checked: false, status: 'pending', planned_start: null, planned_end: null, notes: null, auditeurs: [] }
...
function entityAuds(eid: number | null) {
  ...
  return [...list].sort((a: any, b: any) =>
    (ord[a.role_code ?? a.role] ?? 9) - (ord[b.role_code ?? b.role] ?? 9)
  )
}

function isAudChk(pid: number, eid: number, audId: number) {
  return (getCfg(pid, eid).auditeurs || []).includes(Number(audId))
}
...
// Cocher tous les auditeurs sur toutes les phases cochées d'une entité
function selectAllAuds(eid: number) {
  ...
  for (const ph of allChkPhases.value) {
    if (isCfgChk(ph.id, eid)) setCfg(ph.id, eid, { auditeurs: [...auds] })
  }
  showToast('Tous les auditeurs assignés aux phases cochées.', 'success')
}
...
function onStartChange(pid: number, eid: number, value: string) {
  ...
  // Cascade sur les phases suivantes
  const phases = allChkPhases.value
  const idx = phases.findIndex((p: any) => p.id === pid)
...
function onEndChange(pid: number, eid: number, value: string) {
  setCfg(pid, eid, { planned_end: value || null })
}
...
function entityDateErrors(eid: number | null) {
  ...
  for (const ph of allChkPhases.value) {
    ...
    if (c.planned_end && entity?.date_fin && c.planned_end > entity.date_fin)
      errs.push({ key: `${ph.id}_${eid}_over`, msg: `"${ph.label}" : dépasse la période` })
    if (c.planned_start) prevS = c.planned_start
  }
  return errs
}

function hasStartErr(pid: number, eid: number) {
  return entityDateErrors(eid).some(e => e.key === `${pid}_${eid}_start`)
}
...
function entityHasDirty(eid: number) {
  return [...dirty.value].some(k => k.endsWith('_' + eid))
}
...
function cascadeEntity(eid: number) {
  ...
  showToast('Dates alignées (cascade).', 'success')
}

function clearEntity(eid: number) {
  for (const ph of allChkPhases.value)
    if (getCfg(ph.id, eid).checked)
      setCfg(ph.id, eid, { planned_start: null, planned_end: null, auditeurs: [], notes: null, status: 'pending' })
  showToast('Entité réinitialisée.', 'warning')
}

// URL de formulaire avec paramètre mission
...
// ══════════════════════════════════════════════════════════════════
//  DRAG & DROP (sous-phases)
// ══════════════════════════════════════════════════════════════════
function canDrop(t: any) {
  if (!draggingPhase.value) return false
  return draggingPhase.value.level > 1 && t.level > 1 &&
    draggingPhase.value._parentId === t._parentId
}
...
function onDrop(e: DragEvent, t: any, g: any) {
  ...
  const gi = localPhases.value.findIndex((gr: any) => gr.phase_type === g.phase_type)
  if (gi === -1) return
...
//  NOTES
// ══════════════════════════════════════════════════════════════════
function openNote(pid: number, eid: number) {
  noteModal.value = { show: true, phaseId: pid, entityId: eid, draft: getCfg(pid, eid).notes || '' }
}
...
async function saveAll() {
  ...
  for (const k of dirty.value) {
    ...
    const c = getCfg(pid, eid)
    // Récupérer le form_code de la phase si disponible
  ...
  try {
    ...
    if (!res.ok) {
      const msg = json?.error || json?.message || 'Erreur serveur'
      showToast('Erreur : ' + msg, 'error')
      return
    }

    // Vider dirty seulement si succès
    dirty.value = new Set()

    // Afficher warnings éventuels du serveur
...
function showToast(msg: string, type = 'success') {
  ...
  clearTimeout(_tt)
  _tt = setTimeout(() => { toast.value.show = false }, 5000)
}
...
function stChip(s: string) {
  return ({ planifiee: 'ca', en_cours: 'cb', terminee: 'cg', annulee: 'cr' } as any)[s] || 'cm'
}
...
function rCls(r: string) {
  return ({ DM: 'r-dm', CM: 'r-cm', AS: 'r-as', AJ: 'r-aj' } as any)[r] || 'r-oth'
}
...
/* ══ Variables ══════════════════════════════════════════════════ */
:root {
  --b: #2563eb; --g: #059669; --a: #d97706; --r: #dc2626;
  --bd: #e2e8f0; --sf: #f8fafc; --ink: #0f172a; --m: #64748b;
}
...
.mi-color-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
.mi-code {
  font-family: monospace; font-size: .68rem; font-weight: 800;
  background: #dbeafe; color: #1d4ed8; padding: 2px 7px; border-radius: 4px; flex-shrink: 0;
}
.mi-lib { font-size: .76rem; font-weight: 600; color: #334155; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.mi-badge {
  font-size: .62rem; font-weight: 700; background: #f1f5f9; color: #64748b;
...
.act-danger-ghost { background: #fef2f2; border-color: #fecaca; color: #dc2626; }
...
.tb-search {
  ...
  padding: 0 8px; flex: 1; min-width: 160px; max-width: 300px;
}
...
.prog-bar-wrap { display: flex; align-items: center; gap: 5px; min-width: 80px; }
.prog-bar { flex: 1; height: 5px; background: #e2e8f0; border-radius: 3px; overflow: hidden; }
.prog-fill { height: 100%; background: #059669; border-radius: 3px; transition: width .3s; }
.prog-pct { font-size: .65rem; font-weight: 700; color: #059669; white-space: nowrap; }
...
.s2-layout {
  ...
  padding: 10px;
  gap: 10px;
  /* ← critique : sans ça, les enfants flex ignorent la hauteur du parent */
  overflow: hidden;
  min-height: 0;
}
.s2-side {
  ...
  overflow-y: auto;
  /* hauteur contrainte pour que overflow-y: auto fonctionne */
  min-height: 0;
  max-height: 100%;
}
...
.badge-count {
  ...
  width: 18px; height: 18px; border-radius: 50%;
  background: #2563eb; color: #fff; font-size: .58rem; font-weight: 800;
  margin-left: 4px;
}
.forms-mini-list { display: flex; flex-direction: column; gap: 2px; }
.form-mini-item {
  display: flex; align-items: center; gap: 4px;
  font-size: .68rem; padding: 2px 4px; border-radius: 4px; background: #f8fafc;
}
.fmi-child { padding-left: 12px; color: #94a3b8; }
.fmi-icon { font-size: .72rem; color: #64748b; flex-shrink: 0; }
.fmi-code { font-family: monospace; font-size: .6rem; font-weight: 800; color: #2563eb; flex-shrink: 0; }
.fmi-label { font-size: .65rem; color: #334155; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.form-mini-more { font-size: .65rem; color: #94a3b8; font-style: italic; padding: 2px 4px; }
.forms-source-badge {
  display: flex; align-items: center; gap: 3px;
  font-size: .6rem; color: #94a3b8; margin-top: 2px;
}
.no-forms-hint {
  display: flex; align-items: center; gap: 5px;
...
.s2-groups-wrap {
  /* conteneur principal : prend tout l'espace restant, empile vertical */
  flex: 1;
  display: flex;
  flex-direction: column;
  min-height: 0;        /* ← obligatoire pour que flex enfant puisse scroller */
  overflow: hidden;
}
.s2-groups {
  /* zone scrollable uniquement pour les cards */
  flex: 1;
  overflow-y: auto;
  display: flex;
  flex-direction: column;
...
.loading-phases {
  ...
  gap: 8px; color: #64748b; font-size: .82rem;
}
.empty-phases {
  flex: 1; display: flex; flex-direction: column; align-items: center;
  justify-content: center; gap: 8px; color: #94a3b8;
}
.phases-search-bar {
  display: flex; align-items: center; gap: 6px;
...
.pg-head {
  display: flex; align-items: center; gap: 6px;
  padding: 6px 10px; background: #f8fafc; cursor: pointer; user-select: none;
}
...
.ph-row {
  ...
  cursor: pointer; transition: background .1s; min-height: 30px;
}
...
.ph-form-badge {
  display: inline-flex; align-items: center; gap: 3px;
  font-size: .6rem; font-weight: 700; color: #2563eb;
  background: #dbeafe; padding: 1px 5px; border-radius: 4px; flex-shrink: 0;
}
...
.warn-pill {
  ...
  background: #fffbeb; border: 1px solid #fde68a; padding: 2px 7px; border-radius: 20px;
  cursor: pointer;
}
...
.wp-head {
  display: flex; align-items: center; gap: 6px;
  padding: 5px 12px; font-size: .75rem; font-weight: 700; color: #d97706;
}
.wp-close { margin-left: auto; background: none; border: none; cursor: pointer; color: #d97706; }
.wp-list { padding: 0 12px 8px 30px; font-size: .72rem; color: #92400e; }
.wp-list li { margin-bottom: 2px; }
...
.aff-tbl thead th {
  ...
  background: #0f172a; padding: 6px 5px;
...
.col-form { width: 90px; text-align: center !important; }
.col-aff { width: 40px; }
...
.col-aud { width: 44px; min-width: 40px; padding: 3px 2px !important; }
.col-note { width: 30px; }
...
.tr-sep td {
  ...
  border-bottom: 1px solid #e2e8f0; font-size: .7rem;
}
.sep-icon { margin-right: 4px; }
.sep-cnt { margin-left: 6px; }

/* Colonne statut */
.td-status { padding: 2px 3px !important; }
.status-sel {
  width: 100%; border: 1px solid #e2e8f0; border-radius: 4px;
  padding: 2px 4px; font-size: .65rem; background: #fafafa;
...
.form-link:hover { background: #2563eb; color: #fff; }
.form-tag {
  display: inline-flex; align-items: center; gap: 3px;
...
/* Ligne TOTAL */
.tr-total td {
  padding: 6px 5px; background: #0f172a;
  border-top: 2px solid #334155; vertical-align: middle;
}
...
.tr-warn td { background: #fef2f2 !important; }
...
.no-ent {
  ...
  color: #94a3b8; font-size: .82rem; gap: 6px; background: #fff;
}
...
.modal-box {
  background: #fff; border-radius: 10px; width: 420px; max-width: 94vw;
  box-shadow: 0 20px 60px rgba(0,0,0,.2); border: 1px solid #e2e8f0; overflow: hidden;
}
.modal-hd {
  display: flex; align-items: center; gap: 6px;
  padding: 11px 14px; font-size: .84rem; font-weight: 700; color: #0f172a;
...
.modal-ta {
  ...
  font-family: inherit; font-size: .8rem; color: #334155;
...
.aff-wrap::-webkit-scrollbar, .s1-table-wrap::-webkit-scrollbar,
.s2-groups::-webkit-scrollbar, .s2-side::-webkit-scrollbar { width: 4px; height: 4px; }
.aff-wrap::-webkit-scrollbar-track, .s1-table-wrap::-webkit-scrollbar-track,
.s2-groups::-webkit-scrollbar-track, .s2-side::-webkit-scrollbar-track { background: transparent; }
.aff-wrap::-webkit-scrollbar-thumb, .s1-table-wrap::-webkit-scrollbar-thumb,
.s2-groups::-webkit-scrollbar-thumb, .s2-side::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 2px; }
@media (max-width: 768px) {
  .paf-shell { height: 100vh; border-radius: 0; }
  .s2-layout { flex-direction: column; overflow-y: auto; }
  .s2-side { width: 100%; flex-direction: row; flex-wrap: wrap; max-height: 200px; }
  .s2-groups-wrap { min-height: 300px; }
}
</style>