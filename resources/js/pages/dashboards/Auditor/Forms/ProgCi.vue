    ...
    <div class="container-fluid py-3">
      ...
      <div class="card border-0 shadow-sm mb-3">
          ...
          <div class="d-flex align-items-start gap-2">
            ...
            <div class="flex-grow-1">
              <div class="d-flex align-items-center gap-2 flex-wrap mb-1">
                ...
                <span class="badge bg-warning text-dark">Programme CI</span>
              ...
              <h6 class="mb-0 fw-bold">Programme de Travail d'Audit — Contrôle Interne</h6>
        ...
        <div v-if="form.validation_status === 'validated'"
          ...
          <i class="ti ti-lock"></i> Programme CI <strong>validé définitivement</strong> — lecture seule
        </div>
      ...
      <div class="d-flex align-items-center gap-2 flex-wrap mb-3">
        <div class="alert py-2 px-3 mb-0 small flex-grow-1"
          ...
          <span v-if="sourceLabel === 'RADO+RCI'">
            ...
            Tests liés au <strong>Référentiel CI</strong> (col. H et J).
          </span>
          <span v-else-if="sourceLabel === 'RCI'">
            Objectifs issus du <strong>Référentiel CI</strong> (aucun RADO disponible).
          </span>
          <span v-else>
            <i class="ti ti-alert-triangle text-warning"></i>
            Aucun RADO ni RCI trouvé. Complétez ces formulaires ou importez un Excel.
          </span>
        </div>
      ...
      <div v-if="!objectifs.length" class="alert alert-warning">
        ...
        Aucun objectif disponible. Vérifiez que le <strong>Rapport d'Orientation</strong>
        contient des axes d'audit (générés par l'IA), et que le <strong>Référentiel CI</strong>
        est renseigné.
      </div>
            ...
            <thead>
              ...
              <tr>
                <td :colspan="isLocked ? 11 : 12" class="small py-1 px-2 fst-italic"
                  ...
                  <strong>Test</strong> = col. H du RCI · <strong>Procédures</strong> = col. J du RCI
                </td>
              </tr>
              <tr>
                <th colspan="3" class="text-center text-white small py-1" style="background:#1e40af">Objectif d'Audi</th>
              ...
              <tr>
                ...
                <th class="text-center small" style="background:#1e40af;color:#fff;width:85px">Réf.<br/>RCI</th>
                <th class="small" style="background:#6d28d9;color:#fff;min-width:190px">
                  Test d'Audit
                  <div style="font-size:.58rem;opacity:.75;font-weight:400">↑ RCI col. H</div>
                </th>
                <th class="small" style="background:#6d28d9;color:#fff;min-width:200px">
                  Procédures d'Audit
                  <div style="font-size:.58rem;opacity:.75;font-weight:400">↑ RCI col. J</div>
                </th>
                  ...
                  <tr :class="oi % 2 === 0 ? '' : 'table-light'">
                    ...
                    <td class="align-top px-2 py-2">
                      <div class="d-flex align-items-center gap-1 mb-1">
                        ...
                        <span v-if="test._ia_loading" class="badge bg-light border text-primary" style="font-size:.55rem">
                        ...
                        <div v-if="!isLocked" class="ms-auto d-flex gap-1">
                          <button class="btn btn-sm"
                                  ...
                                  :disabled="test._ia_loading || aiAutoRunning"
                                  title="Reformuler par IA"
                      ...
                      <!-- Suggestions IA -->
                      <div v-if="test._ia_suggestions?.length" class="mt-2 pt-2 border-top" style="border-color:#c7d2fe!important">
                        ...
                        <div v-for="(s, si) in test._ia_suggestions" :key="si"
                             ...
                             style="background:#eef2ff;border:1px solid #c7d2fe;font-size:.67rem;cursor:pointer;transition:background .15s"
                          ...
                          <strong style="color:#4338ca">{{ s.ref }}</strong> — {{ s.libelle }}
                          <span v-if="s.procedures?.length" class="ms-1 badge bg-secondary" style="font-size:.52rem">
                    ...
                    <td class="text-center align-middle px-1 py-1" >
                      <input v-if="!isLocked" type="text" class="form-control form-control-sm text-center"
                             ...
                             placeholder="Periode testee" style="font-size:.7rem" />
...
const props = withDefaults(defineProps<{
  ...
  urlValider?:       string
  urlAiSuggest?:     string
  urlAiSuggestBase?: string
  urlAiSynthese?:    string
  urlUpload?:        string
  urlBase?:          string
...
const dynUrls = reactive({
  ...
  aiSynthese: props.urlAiSynthese ?? null as string | null,
...
//
// Priorité :
//   1. Si donneesRCI vient du RADO (source = rado+rci) → TOUJOURS utiliser donneesRCI
//      même si form.lignes existe (les lignes sauvegardées peuvent venir de l'ancien RCI seul)
//   2. Si donneesRCI vient du RCI seul ET form.lignes existe → utiliser form.lignes
//      (l'auditeur a édité ses lignes RCI → on respecte ses éditions)
//   3. Sinon → donneesRCI (fallback)
// ══════════════════════════════════════════════════════════════
function initObjectifs() {
  ...
  if (hasRado && fromRCI.length) {
    // RADO disponible → utiliser toujours les données fraîches du RADO+RCI
    source = fromRCI
  } else if (!hasRado && fromForm.length) {
    // Pas de RADO, mais l'auditeur a déjà édité ses lignes → respecter ses éditions
    source = fromForm
  ...
  source.forEach(o => objectifs.push(hydrateObj(o)))
}
initObjectifs()
...
function hydrateTest(t: any) {
  return {
    ...t, _tuid: tuid(), _edited: false, _ia_loading: false,
...
function urlAI(): string | null {
  ...
  if (form.id && props.urlAiSuggestBase) return `${props.urlAiSuggestBase}/${form.id}/ai-suggest`
  if (form.id && props.urlBase)          return `${props.urlBase}/${form.id}/ai-suggest`
  return null
}

onMounted(async () => {
  if (isLocked.value || !objectifs.length) return
  if (!objectifs.some(o => o._needs_ai)) return

  if (!form.id) {
    showToast('info', 'Sauvegarde automatique avant génération IA…', 8000)
    const ok = await autoSave()
    if (!ok) { showToast('error', 'Sauvegarde auto échouée — utilisez les boutons IA manuellement.'); return }
  }

  aiAutoRunning.value = true
  let nbTraites = 0
  for (const obj of objectifs) {
    if (!obj._needs_ai) continue
    await aiReformuler(obj)
    nbTraites++
  }
  ...
  if (nbTraites > 0) {
    await submit(true)
    showToast('success', `IA : ${nbTraites} objectif(s) reformulé(s) et sauvegardés.`)
  }
})
...
async function autoSave(): Promise<boolean> {
  ...
  try {
    ...
    if ((d.success || res.ok) && d.form) {
      ...
      if (d.urlAiSynthese) dynUrls.aiSynthese = d.urlAiSynthese
...
async function aiReformuler(obj: any) {
  ...
  try {
    const r = await callAI(url, { objectif: buildObjPayload(obj, obj.tests), mode: 'reformuler', context: ctxIA(obj) })
    if (r.success && Array.isArray(r.tests) && r.tests.length) {
      obj.tests.splice(0, obj.tests.length, ...r.tests.map((t: any) => hydrateTest(t)))
      obj._needs_ai = false; obj._edited = true
    }
  } catch (e) { console.warn('[PTCI] aiReformuler:', e) }
  finally { obj.tests.forEach((t: any) => { t._ia_loading = false }) }
}

async function reformulerTest(obj: any, test: any) {
  const url = urlAI()
  if (!url) { showToast(form.id ? 'error' : 'info', form.id ? 'URL IA indisponible.' : 'Enregistrez d\'abord le formulaire.'); return }
  test._ia_loading = true; test._ia_suggestions = null
  try {
    const r = await callAI(url, { objectif: buildObjPayload(obj, [test]), mode: 'reformuler', context: ctxIA(obj) })
    if (r.success && Array.isArray(r.tests) && r.tests.length) test._ia_suggestions = r.tests
    else showToast('error', r.error ?? 'Aucune suggestion générée.')
...
function appliquerSuggestion(test: any, s: any) {
  test.libelle = s.libelle; test.procedures = Array.isArray(s.procedures) ? [...s.procedures] : []
  test._ia_suggestions = null; test._edited = true; showToast('success', 'Suggestion appliquée.')
}
...
function buildObjPayload(obj: any, tests: any[]) {
  return {
    num: obj.num, objectif: obj.objectif, ref_rci: obj.ref_rci, tests,
    _axe_rado: obj._axe_rado ?? '', _indicateurs: obj._indicateurs ?? '',
    _risque_code: obj._risque_code ?? '', _risque_libelle: obj._risque_libelle ?? '',
    _process_name: obj._process_name ?? '', _objectif_operationnel: obj._objectif_operationnel ?? '',
    _description_controle: obj._description_controle ?? '', _preuve_controle: obj._preuve_controle ?? '',
    _type_controle: obj._type_controle ?? '', _criticite: obj._criticite ?? 0, _responsable: obj._responsable ?? '',
  }
}

function ctxIA(obj: any) {
  return {
    ...(props.missionContext ?? {}),
    axe_rado: obj._axe_rado ?? '', indicateurs: obj._indicateurs ?? '',
    risque_code: obj._risque_code ?? '', risque_libelle: obj._risque_libelle ?? '',
    processus: obj._process_name ?? props.missionContext?.processus ?? '',
    objectif_operationnel: obj._objectif_operationnel ?? '',
    type_controle: obj._type_controle ?? '', criticite: obj._criticite ?? 0,
    responsable: obj._responsable ?? '', obj_num: obj.num ?? '',
  }
}
...
async function submit(silent = false) {
  ...
  try {
    ...
    const d = await res.json()
    if (d.success || res.ok) {
      if (!silent) showToast('success', form.id ? 'Programme CI mis à jour.' : 'Programme CI créé.')
...
async function valider(action: string, note?: string) {
  ...
  try {
    ...
    if (d.success) { form.validation_status = d.status; showToast('success', action === 'validate' ? 'Programme CI validé ✓' : 'Rejeté.') }
    else showToast('error', d.error ?? 'Erreur')
  } catch { showToast('error', 'Erreur réseau') }
  processing.value = false
}
