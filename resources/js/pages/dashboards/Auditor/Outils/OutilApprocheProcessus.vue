    ...
    <div class="oz-shell">
        ...
        <div class="oz-header__inner">
          ...
          <div class="oz-header__meta">
            <div class="oz-header__badges">
              <span class="oz-code">{{ form.code||'IV-AUTO' }}</span>
            ...
            <h1 class="oz-header__title" style="--hc:#b45309"><span class="oz-num">IV</span>Approche Processus</h1>
      ...
      <div class="oz-body">
        ...
        <div class="oz-card">
          ...
          <div class="oz-g2">
            <div class="oz-f oz-full"><label class="oz-lbl">Intitulé *</label><input type="text" class="oz-inp" v-model="form.intitule" :disabled="isLocked" placeholder="Cartographie Processus Achats"/></div>
            <div class="oz-f"><label class="oz-lbl">Domaine audité</label><input type="text" class="oz-inp" v-model="form.domaine" :disabled="isLocked"/></div>
        ...
        <div v-for="tp in typesProc" :key="tp.code" class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title" :style="'color:'+tp.color"><i :class="tp.icon"></i>{{tp.label}}</h3>
            <small class="oz-muted">{{tp.desc}}</small>
            <span class="oz-badge-count">{{ getLignes(tp.code).length }}</span>
            <button v-if="!isLocked" class="oz-add" @click="addLigne(tp.code)"><i class="ti ti-plus"></i> Processus</button>
          </div>
          <div class="oz-table-wrap">
            <table class="oz-tbl">
              <thead><tr><th class="tc" style="width:28px">N°</th><th style="min-width:150px">Nom Processus</th><th style="min-width:120px">Finalité</th><th style="min-width:110px">Él. Entrants</th><th style="min-width:110px">Él. Sortants</th><th style="min-width:120px">Activités princ.</th><th style="min-width:100px">Clients</th><th style="min-width:100px">Fournisseurs</th><th v-if="!isLocked" style="width:28px"></th></tr></thead>
              <tbody>
                <tr v-if="!getLignes(tp.code).length"><td colspan="9" class="oz-ec">Aucun processus</td></tr>
                <tr v-for="(l,li) in getLignes(tp.code)" :key="l._id">
                  <td class="tc oz-n">{{li+1}}</td>
                  <td><input class="oz-inp-sm" type="text" v-model="l.libelle" :disabled="isLocked" placeholder="Nom…"/></td>
                  <td><textarea class="oz-ta-sm" v-model="l.finalite" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.elements_entrants" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.elements_sortants" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.activites_princ" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.clients" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.fournisseurs" rows="2" :disabled="isLocked"></textarea></td>
                  <td v-if="!isLocked" class="tc"><button class="oz-del" @click="delLigne(l._id)"><i class="ti ti-trash"></i></button></td>
                </tr>
              </tbody>
            </table>
          </div>
...
const form=reactive<any>({id:null,code:'',statut:'draft',validation_note:'',intitule:'',domaine:'',date_analyse:'',...(props.record??{})})
let _id=0;const lignes=reactive<any[]>((props.lignes??[]).map((l:any)=>({...l,_id:_id++})))
const processing=ref(false)
const typesProc=[{code:'realisation',label:'1. Processus de Réalisation',icon:'ti ti-arrow-right',color:'#065f46',desc:'Produisent des produits/services'},{code:'management',label:'2. Processus de Management',icon:'ti ti-chart-line',color:'#1e40af',desc:'Pilotage & direction'},{code:'support',label:'3. Processus de Support',icon:'ti ti-tool',color:'#6d28d9',desc:'Soutien aux processus'}]
function getLignes(type:string){return lignes.filter(l=>l.type===type)}
function addLigne(type:string){lignes.push({type,libelle:'',finalite:'',elements_entrants:'',elements_sortants:'',activites_princ:'',clients:'',fournisseurs:'',contrats_interface:'',_id:_id++})}
function delLigne(id:number){const i=lignes.findIndex(l=>l._id===id);if(i>-1)lignes.splice(i,1)}
function buildPayload(){return{mission_id:props.missionContext?.mission_id,assignment_id:props.missionContext?.assignment_id,procedure_code:props.missionContext?.procedure_code,test_ref:props.missionContext?.test_ref,intitule:form.intitule,domaine:form.domaine,date_analyse:form.date_analyse,lignes:lignes.map(({_id,...r}:any)=>r)}}
