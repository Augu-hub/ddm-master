    ...
    <div class="oz-shell">
        ...
        <div class="oz-header__inner">
          ...
          <div class="oz-header__meta">
            <div class="oz-header__badges">
              <span class="oz-code">{{ form.code||'II-AUTO' }}</span>
            ...
            <h1 class="oz-header__title" style="--hc:#065f46"><span class="oz-num">II</span>Grille d'Analyse des Tâches</h1>
      ...
      <div class="oz-body">
        ...
        <div class="oz-card">
          ...
          <div class="oz-g3">
            <div class="oz-f oz-full"><label class="oz-lbl">Intitulé *</label><input type="text" class="oz-inp" v-model="form.intitule" :disabled="isLocked" placeholder="Ex. : Analyse tâches Comptabilité Fournisseurs"/></div>
            <div class="oz-f"><label class="oz-lbl">Processus audité</label><input type="text" class="oz-inp" v-model="form.processus" :disabled="isLocked" placeholder="Achats / Fournisseurs"/></div>
            <div class="oz-f"><label class="oz-lbl">Responsable</label><input type="text" class="oz-inp" v-model="form.responsable" :disabled="isLocked"/></div>
        ...
        <div class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title"><i class="ti ti-users"></i> Acteurs (colonnes)</h3>
            <span class="oz-badge-count">{{ acteurs.length }}</span>
            <button v-if="!isLocked&&acteurs.length<8" class="oz-add" @click="acteurs.push({nom:''})"><i class="ti ti-plus"></i> Acteur</button>
          </div>
          <div class="oz-at-row">
            <div v-for="(a,ai) in acteurs" :key="ai" class="oz-at-item">
              <span class="oz-at-badge" :style="'background:'+actC[ai%8]">{{ai+1}}</span>
              <input class="oz-inp-sm" style="width:110px" type="text" v-model="a.nom" :disabled="isLocked" placeholder="Nom acteur"/>
              <button v-if="!isLocked" class="oz-del" @click="acteurs.splice(ai,1)"><i class="ti ti-x"></i></button>
            </div>
          </div>
          <p style="font-size:.6rem;color:#94a3b8;margin:.3rem 0 0">R=Réalise · A=Approuve · C=Consulté · I=Informé</p>
        </div>
        <div class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title"><i class="ti ti-layout-list"></i> Tâches du Processus</h3>
            <span class="oz-badge-count">{{ lignes.length }}</span>
            <button v-if="!isLocked" class="oz-add" @click="addLigne"><i class="ti ti-plus"></i> Tâche</button>
          </div>
          <div class="oz-table-wrap">
            <table class="oz-tbl">
              <thead><tr>
                <th class="tc" style="width:28px">N°</th><th style="min-width:200px">Tâche du Processus</th>
                <th v-for="(a,ai) in acteurs" :key="ai" style="width:46px" class="tc">
                  <span :style="'display:inline-block;padding:1px 3px;border-radius:3px;background:'+actC[ai%8]+';color:#fff;font-size:.58rem;font-weight:700;max-width:44px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap'">{{a.nom||(''+(ai+1))}}</span>
                </th>
                <th style="min-width:120px">Risque associé</th><th style="min-width:110px">Contrôle</th><th style="min-width:100px">Observation</th>
                <th v-if="!isLocked" style="width:28px"></th>
              </tr></thead>
              <tbody>
                <tr v-if="!lignes.length"><td :colspan="5+acteurs.length" class="oz-ec">Aucune tâche</td></tr>
                <tr v-for="(l,li) in lignes" :key="li">
                  <td class="tc oz-n">{{li+1}}</td>
                  <td><textarea class="oz-ta-sm" v-model="l.libelle" rows="2" :disabled="isLocked" placeholder="Description…"></textarea></td>
                  <td v-for="(a,ai) in acteurs" :key="ai" class="tc">
                    <select v-if="!isLocked" class="oz-sel-sm" style="width:36px;text-align:center" v-model="l.roles[ai]">
                      <option value="">—</option><option>R</option><option>A</option><option>C</option><option>I</option>
                    </select>
                    <span v-else :class="'orb--'+(l.roles[ai]||'')" style="font-weight:700;font-size:.7rem">{{l.roles[ai]||'—'}}</span>
                  </td>
                  <td><textarea class="oz-ta-sm" v-model="l.risque" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.controle" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="l.observation" rows="2" :disabled="isLocked"></textarea></td>
                  <td v-if="!isLocked" class="tc"><button class="oz-del" @click="lignes.splice(li,1)"><i class="ti ti-trash"></i></button></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-alert-triangle"></i> Observations — Séparation des Fonctions Incompatibles</h3>
          <textarea class="oz-ta" v-model="form.observations" :disabled="isLocked" rows="3" placeholder="Incompatibilités constatées…"></textarea>
        </div>
...
const props=withDefaults(defineProps<{record?:any;lignes?:any[];missions?:any[];auditorRole?:string;auditeurNom?:string;backUrl?:string;urlStore?:string;urlUpdate?:string|null;urlSoumettre?:string|null;urlValider?:string|null;urlIa?:string|null;missionContext?:any;}>(),{record:null,lignes:()=>[],missions:()=>[],auditorRole:'AJ',auditeurNom:'',backUrl:'/',urlStore:'',urlUpdate:null,urlSoumettre:null,urlValider:null,urlIa:null,missionContext:()=>({})})
const form=reactive<any>({id:null,code:'',statut:'draft',validation_note:'',intitule:'',processus:'',responsable:'',date_analyse:'',observations:'',...(props.record??{})})
const raw=props.record?.acteurs_json;const acteurs=reactive<any[]>(raw?(typeof raw==='string'?JSON.parse(raw):raw):[{nom:''},{nom:''},{nom:''}])
const lignes=reactive<any[]>([...(props.lignes??[]).map((l:any)=>({...l,roles:l.roles_json?(typeof l.roles_json==='string'?JSON.parse(l.roles_json):l.roles_json):[]}))])
const processing=ref(false)
const actC=['#1e40af','#065f46','#6d28d9','#b45309','#be185d','#0369a1','#0f766e','#7c3aed']
function addLigne(){lignes.push({libelle:'',roles:acteurs.map(()=>''),risque:'',controle:'',observation:''})}
function buildPayload(){return{mission_id:props.missionContext?.mission_id,assignment_id:props.missionContext?.assignment_id,procedure_code:props.missionContext?.procedure_code,test_ref:props.missionContext?.test_ref,intitule:form.intitule,processus:form.processus,responsable:form.responsable,date_analyse:form.date_analyse,observations:form.observations,acteurs_json:acteurs,lignes:lignes.map((l:any)=>({...l,roles_json:l.roles}))}}
...
.oz-at-row{display:flex;flex-wrap:wrap;gap:.4rem;margin-bottom:.5rem;}.oz-at-item{display:flex;align-items:center;gap:.3rem;}.oz-at-badge{display:inline-flex;align-items:center;justify-content:center;min-width:20px;height:18px;border-radius:3px;color:#fff;font-size:.6rem;font-weight:700;}.orb--R{color:#1d4ed8;font-weight:700;font-size:.7rem;}.orb--A{color:#15803d;font-weight:700;font-size:.7rem;}.orb--C{color:#d97706;font-weight:700;font-size:.7rem;}.orb--I{color:#6b7280;font-weight:700;font-size:.7rem;}
</style>
