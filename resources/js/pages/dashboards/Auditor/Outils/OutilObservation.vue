    ...
    <div class="oz-shell">
        ...
        <div class="oz-header__inner">
          ...
          <div class="oz-header__meta">
            <div class="oz-header__badges">
              <span class="oz-code">{{ form.code||'XIV-AUTO' }}</span>
            ...
            <h1 class="oz-header__title" style="--hc:#9333ea"><span class="oz-num">XIV</span>Fiche d'Observation Directe</h1>
      ...
      <div class="oz-body">
        ...
        <div class="oz-card">
          ...
          <div class="oz-g3">
            <div class="oz-f"><label class="oz-lbl">Date d'observation</label><input type="date" class="oz-inp" v-model="form.date_observation" :disabled="isLocked"/></div>
            <div class="oz-f"><label class="oz-lbl">Heure début</label><input type="time" class="oz-inp" v-model="form.heure_debut" :disabled="isLocked"/></div>
            <div class="oz-f"><label class="oz-lbl">Heure fin</label><input type="time" class="oz-inp" v-model="form.heure_fin" :disabled="isLocked"/></div>
            ...
            <div class="oz-f"><label class="oz-lbl">Localisation</label><input type="text" class="oz-inp" v-model="form.localisation" :disabled="isLocked" placeholder="Ex. : Magasin central — Porte B"/></div>
            <div class="oz-f oz-full"><label class="oz-lbl">Interlocuteur(s) présent(s)</label><input type="text" class="oz-inp" v-model="form.interlocuteurs_presents" :disabled="isLocked" placeholder="Noms, fonctions…"/></div>
          </div>
        </div>
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-clipboard"></i> Préparation de l'Observation</h3>
          <div class="oz-g2">
            <div class="oz-f oz-full"><label class="oz-lbl">Objectif d'audit visé</label><textarea class="oz-ta" v-model="form.objectif_audit" :disabled="isLocked" rows="2" placeholder="S'assurer que…"></textarea></div>
            <div class="oz-f oz-full"><label class="oz-lbl">Tâche / Local à observer</label><textarea class="oz-ta" v-model="form.tache_local_observer" :disabled="isLocked" rows="2" placeholder="Description précise de ce qui sera observé…"></textarea></div>
            <div class="oz-f oz-full"><label class="oz-lbl">Éléments à vérifier</label><textarea class="oz-ta" v-model="form.elements_verifier" :disabled="isLocked" rows="2" placeholder="Liste des éléments à vérifier durant l'observation…"></textarea></div>
            <div class="oz-f oz-full"><label class="oz-lbl">Pièces attendues</label><textarea class="oz-ta" v-model="form.pieces_attendues" :disabled="isLocked" rows="2" placeholder="Documents, registres, équipements attendus sur place…"></textarea></div>
          </div>
        </div>
        <div class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title"><i class="ti ti-eye"></i> Constats d'Observation</h3>
            <span class="oz-badge-count">{{ constats.length }}</span>
            <button v-if="!isLocked" class="oz-add" @click="constats.push({element_observe:'',conforme_referentiel:'',ecart_constate:'',risque_associe:'',preuve:''})"><i class="ti ti-plus"></i> Constat</button>
          </div>
          <div class="oz-table-wrap">
            <table class="oz-tbl">
              <thead><tr><th class="tc" style="width:28px">N°</th><th style="min-width:180px">Élément observé</th><th class="tc" style="width:90px">Conforme?</th><th style="min-width:150px">Écart constaté</th><th style="min-width:130px">Risque associé</th><th style="width:110px">Preuve (Photo/Doc/Note)</th><th v-if="!isLocked" style="width:28px"></th></tr></thead>
              <tbody>
                <tr v-if="!constats.length"><td colspan="7" class="oz-ec">Aucun constat</td></tr>
                <tr v-for="(c,ci) in constats" :key="ci" :style="c.conforme_referentiel==='non'?'background:#fef2f2':c.conforme_referentiel==='oui'?'background:#f0fdf4':c.conforme_referentiel==='partiel'?'background:#fffbeb':''">
                  <td class="tc oz-n">{{ci+1}}</td>
                  <td><textarea class="oz-ta-sm" v-model="c.element_observe" rows="2" :disabled="isLocked" placeholder="Élément observé…"></textarea></td>
                  <td class="tc"><select class="oz-sel-sm" v-model="c.conforme_referentiel" :disabled="isLocked" :style="c.conforme_referentiel==='oui'?'color:#15803d;font-weight:700':c.conforme_referentiel==='non'?'color:#dc2626;font-weight:700':c.conforme_referentiel==='partiel'?'color:#d97706;font-weight:700':''"><option value="">—</option><option value="oui">✅ Oui</option><option value="non">❌ Non</option><option value="partiel">⚠️ Partiel</option></select></td>
                  <td><textarea class="oz-ta-sm" v-model="c.ecart_constate" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="c.risque_associe" rows="2" :disabled="isLocked"></textarea></td>
                  <td><input class="oz-inp-sm" type="text" v-model="c.preuve" :disabled="isLocked" placeholder="Réf. photo, doc…"/></td>
                  <td v-if="!isLocked" class="tc"><button class="oz-del" @click="constats.splice(ci,1)"><i class="ti ti-trash"></i></button></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-thumb-up"></i> Points Forts Observés</h3>
          <textarea class="oz-ta" v-model="form.points_forts" :disabled="isLocked" rows="3" placeholder="Décrire les points positifs observés…"></textarea>
        </div>
        <div class="oz-card" style="border-color:#fecaca">
          <h3 class="oz-card__title"><i class="ti ti-thumb-down"></i> Points Faibles / Dysfonctionnements Observés</h3>
          <textarea class="oz-ta" v-model="form.points_faibles" :disabled="isLocked" rows="3" placeholder="Décrire les points faibles et dysfonctionnements observés…"></textarea>
        </div>
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-file-text"></i> Conclusion et Suites à Donner</h3>
          <textarea class="oz-ta" v-model="form.conclusion" :disabled="isLocked" rows="3" placeholder="Conclusion générale et suites à donner…"></textarea>
        </div>
...
const props=withDefaults(defineProps<{record?:any;constats?:any[];missions?:any[];auditorRole?:string;auditeurNom?:string;backUrl?:string;urlStore?:string;urlUpdate?:string|null;urlSoumettre?:string|null;urlValider?:string|null;urlIa?:string|null;missionContext?:any;}>(),{record:null,constats:()=>[],missions:()=>[],auditorRole:'AJ',auditeurNom:'',backUrl:'/',urlStore:'',urlUpdate:null,urlSoumettre:null,urlValider:null,urlIa:null,missionContext:()=>({})})
const form=reactive<any>({id:null,code:'',statut:'draft',validation_note:'',date_observation:'',heure_debut:'',heure_fin:'',auditeur:props.auditeurNom,localisation:'',interlocuteurs_presents:'',objectif_audit:'',tache_local_observer:'',elements_verifier:'',pieces_attendues:'',points_forts:'',points_faibles:'',conclusion:'',...(props.record??{})})
const constats=reactive<any[]>([...(props.constats??[])])
const processing=ref(false)
function buildPayload(){return{mission_id:props.missionContext?.mission_id,assignment_id:props.missionContext?.assignment_id,procedure_code:props.missionContext?.procedure_code,test_ref:props.missionContext?.test_ref,date_observation:form.date_observation,heure_debut:form.heure_debut,heure_fin:form.heure_fin,auditeur:form.auditeur,localisation:form.localisation,interlocuteurs_presents:form.interlocuteurs_presents,objectif_audit:form.objectif_audit,tache_local_observer:form.tache_local_observer,elements_verifier:form.elements_verifier,pieces_attendues:form.pieces_attendues,points_forts:form.points_forts,points_faibles:form.points_faibles,conclusion:form.conclusion,constats}}
