    ...
    <div class="oz-shell">
        ...
        <div class="oz-header__inner">
          ...
          <div class="oz-header__meta">
            <div class="oz-header__badges">
              <span class="oz-code">{{ form.code||'X-AUTO' }}</span>
            ...
            <h1 class="oz-header__title" style="--hc:#d97706"><span class="oz-num">X</span>Brainstorming (Remue-méninges)</h1>
      ...
      <div class="oz-body">
        ...
        <div class="oz-card">
          ...
          <div class="oz-g2">
            <div class="oz-f oz-full"><label class="oz-lbl">Dysfonctionnement / Sujet traité <span class="oz-req">*</span></label>
              <input type="text" class="oz-inp" style="border-color:#d97706;font-weight:600;font-size:.8rem;padding:6px 10px" v-model="form.problematique" :disabled="isLocked" placeholder="Ex. : Retards fréquents dans la saisie des factures fournisseurs…"/>
            </div>
            <div class="oz-f oz-full"><label class="oz-lbl">Intitulé de la session *</label><input type="text" class="oz-inp" v-model="form.intitule" :disabled="isLocked" placeholder="Ex. : Session brainstorming — Processus Paie"/></div>
            <div class="oz-f"><label class="oz-lbl">Animateur</label><input type="text" class="oz-inp" v-model="form.animateur" :disabled="isLocked" :placeholder="props.auditeurNom"/></div>
            <div class="oz-f"><label class="oz-lbl">Participants</label><input type="text" class="oz-inp" v-model="form.participants" :disabled="isLocked" placeholder="Noms, fonctions…"/></div>
            <div class="oz-f"><label class="oz-lbl">Durée</label><input type="text" class="oz-inp" v-model="form.duree" :disabled="isLocked" placeholder="Ex. : 60 min"/></div>
            <div class="oz-f"><label class="oz-lbl">Date</label><input type="date" class="oz-inp" v-model="form.date_session" :disabled="isLocked"/></div>
          </div>
        </div>
        <div class="oz-card" style="background:#fffbeb;border-color:#fde68a">
          <h3 class="oz-card__title"><i class="ti ti-sparkles"></i> Règles d'or du Brainstorming</h3>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:.4rem;margin-top:.4rem">
            <div v-for="(r,ri) in regles" :key="ri" style="display:flex;align-items:center;gap:.4rem;padding:.35rem .5rem;background:#fff;border-radius:6px;border:1px solid #fde68a">
              <span style="display:inline-flex;align-items:center;justify-content:center;width:18px;height:18px;background:#d97706;color:#fff;border-radius:50%;font-size:.6rem;font-weight:700;flex-shrink:0">{{ri+1}}</span>
              <span style="font-size:.68rem;color:#374151">{{r}}</span>
            </div>
          </div>
        </div>
        <div class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title"><i class="ti ti-bulb"></i> Phase 1 — Idées émises</h3>
            <span class="oz-badge-count">{{ idees.length }}</span>
            <button v-if="!isLocked" class="oz-add" @click="idees.push({libelle:'',emis_par:'',theme:'',categorie:'',votes:0,retenue:false,a_approfondir:false,priorite:0})"><i class="ti ti-plus"></i> Idée</button>
          </div>
          <div class="oz-table-wrap">
            <table class="oz-tbl">
              <thead><tr><th class="tc" style="width:28px">N°</th><th>Idée émise</th><th style="width:100px">Émise par</th><th style="width:90px">Catégorie</th><th class="tc" style="width:55px">Votes</th><th class="tc" style="width:65px">Priorité</th><th class="tc" style="width:65px">Retenue</th><th class="tc" style="width:75px">Approfondir</th><th v-if="!isLocked" style="width:28px"></th></tr></thead>
              <tbody>
                <tr v-if="!idees.length"><td colspan="9" class="oz-ec">Aucune idée — cliquez + pour ajouter</td></tr>
                <tr v-for="(i,ii) in idees" :key="ii" :style="i.retenue?'background:#f0fdf4':''">
                  <td class="tc oz-n">{{ii+1}}</td>
                  <td><textarea class="oz-ta-sm" v-model="i.libelle" rows="2" :disabled="isLocked" placeholder="Idée…"></textarea></td>
                  <td><input class="oz-inp-sm" type="text" v-model="i.emis_par" :disabled="isLocked" placeholder="Nom…"/></td>
                  <td><select class="oz-sel-sm" v-model="i.categorie" :disabled="isLocked"><option value="">—</option><option>Méthode</option><option>Matière</option><option>Main d'Œuvre</option><option>Matériel</option><option>Milieu</option><option>Management</option><option>Risque</option><option>Solution</option></select></td>
                  <td class="tc"><input class="oz-inp-sm" style="width:40px;text-align:center" type="number" v-model.number="i.votes" :disabled="isLocked" min="0"/></td>
                  <td class="tc"><select class="oz-sel-sm" v-model.number="i.priorite" :disabled="isLocked"><option :value="0">—</option><option :value="1">1</option><option :value="2">2</option><option :value="3">3</option></select></td>
                  <td class="tc"><input type="checkbox" v-model="i.retenue" :disabled="isLocked"/></td>
                  <td class="tc"><input type="checkbox" v-model="i.a_approfondir" :disabled="isLocked"/></td>
                  <td v-if="!isLocked" class="tc"><button class="oz-del" @click="idees.splice(ii,1)"><i class="ti ti-trash"></i></button></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-check"></i> Synthèse et Actions Correctives validées par consensus</h3>
          <textarea class="oz-ta" v-model="form.synthese" :disabled="isLocked" rows="4" placeholder="Résumé des idées retenues, actions décidées par consensus…"></textarea>
        </div>
...
const props=withDefaults(defineProps<{record?:any;idees?:any[];missions?:any[];auditorRole?:string;auditeurNom?:string;backUrl?:string;urlStore?:string;urlUpdate?:string|null;urlSoumettre?:string|null;urlValider?:string|null;urlIa?:string|null;missionContext?:any;}>(),{record:null,idees:()=>[],missions:()=>[],auditorRole:'AJ',auditeurNom:'',backUrl:'/',urlStore:'',urlUpdate:null,urlSoumettre:null,urlValider:null,urlIa:null,missionContext:()=>({})})
const form=reactive<any>({id:null,code:'',statut:'draft',validation_note:'',intitule:'',problematique:'',animateur:props.auditeurNom,participants:'',duree:'',date_session:'',synthese:'',...(props.record??{})})
const idees=reactive<any[]>([...(props.idees??[])])
const processing=ref(false)
const regles=["Ne pas critiquer","Ne pas s'autocensurer","Émettre des idées à partir des idées des autres","Être bref","Laisser aller son imagination"]
function buildPayload(){return{mission_id:props.missionContext?.mission_id,assignment_id:props.missionContext?.assignment_id,procedure_code:props.missionContext?.procedure_code,test_ref:props.missionContext?.test_ref,intitule:form.intitule,problematique:form.problematique,animateur:form.animateur,participants:form.participants,duree:form.duree,date_session:form.date_session,synthese:form.synthese,idees}}
