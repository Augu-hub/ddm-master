    ...
    <div class="oz-shell">
        ...
        <div class="oz-header__inner">
          ...
          <div class="oz-header__meta">
            <div class="oz-header__badges">
              <span class="oz-code">{{ form.code||'III-AUTO' }}</span>
            ...
            <h1 class="oz-header__title" style="--hc:#6d28d9"><span class="oz-num">III</span>Diagramme de Flux</h1>
      ...
      <div class="oz-body">
        ...
        <div class="oz-card">
          ...
          <div class="oz-g3">
            <div class="oz-f oz-full"><label class="oz-lbl">Intitulé *</label><input type="text" class="oz-inp" v-model="form.intitule" :disabled="isLocked" placeholder="Ex. : Diagramme Processus Paie"/></div>
            <div class="oz-f"><label class="oz-lbl">Processus</label><input type="text" class="oz-inp" v-model="form.processus" :disabled="isLocked" placeholder="Gestion de la paie"/></div>
            <div class="oz-f"><label class="oz-lbl">Version</label><input type="text" class="oz-inp" v-model="form.version" :disabled="isLocked" placeholder="V1"/></div>
        ...
        <div class="oz-card">
          <div class="oz-card__hd">
            <h3 class="oz-card__title"><i class="ti ti-list-numbers"></i> Étape 1 — Inventaire des Activités et Acteurs</h3>
            <span class="oz-badge-count">{{ activites.length }}</span>
            <button v-if="!isLocked" class="oz-add" @click="activites.push({activite:'',acteur:'',element_entrant:'',element_sortant:'',documents:''})"><i class="ti ti-plus"></i> Activité</button>
          </div>
          <div class="oz-table-wrap">
            <table class="oz-tbl">
              <thead><tr><th class="tc" style="width:28px">N°</th><th style="min-width:180px">Activité</th><th style="min-width:120px">Acteur</th><th style="min-width:110px">Él. Entrant</th><th style="min-width:110px">Él. Sortant</th><th style="min-width:110px">Documents</th><th v-if="!isLocked" style="width:28px"></th></tr></thead>
              <tbody>
                <tr v-if="!activites.length"><td colspan="7" class="oz-ec">Aucune activité</td></tr>
                <tr v-for="(a,ai) in activites" :key="ai">
                  <td class="tc oz-n">{{ai+1}}</td>
                  <td><textarea class="oz-ta-sm" v-model="a.activite" rows="2" :disabled="isLocked" placeholder="Description…"></textarea></td>
                  <td><input class="oz-inp-sm" type="text" v-model="a.acteur" :disabled="isLocked"/></td>
                  <td><textarea class="oz-ta-sm" v-model="a.element_entrant" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="a.element_sortant" rows="2" :disabled="isLocked"></textarea></td>
                  <td><textarea class="oz-ta-sm" v-model="a.documents" rows="2" :disabled="isLocked"></textarea></td>
                  <td v-if="!isLocked" class="tc"><button class="oz-del" @click="activites.splice(ai,1)"><i class="ti ti-trash"></i></button></td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-text-size"></i> Étape 2 — Description Narrative du Processus</h3>
          <textarea class="oz-ta" v-model="form.description_narrative" :disabled="isLocked" rows="5" placeholder="Décrire le déroulement chronologique du processus…"></textarea>
        </div>
        <div class="oz-card" style="background:#f5f3ff;border-color:#ddd6fe">
          <h3 class="oz-card__title"><i class="ti ti-shapes"></i> Étape 3 — Symboles ISO 5807</h3>
          <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(110px,1fr));gap:.4rem;margin-top:.4rem">
            <div v-for="s in isoSymbols" :key="s.n" style="display:flex;align-items:center;gap:.3rem;padding:.3rem .45rem;border:1px solid #e2e8f0;border-radius:5px;background:#fff">
              <span :style="'display:flex;align-items:center;justify-content:center;width:22px;height:17px;border:2px solid '+s.c+';border-radius:2px;font-size:.68rem;font-weight:700;color:'+s.c">{{s.i}}</span>
              <span style="font-size:.6rem;color:#475569">{{s.n}}</span>
            </div>
          </div>
        </div>
        <div class="oz-card">
          <h3 class="oz-card__title"><i class="ti ti-check"></i> Synthèse et Validations</h3>
          <textarea class="oz-ta" v-model="form.synthese_validations" :disabled="isLocked" rows="3" placeholder="Points validés, anomalies, remarques…"></textarea>
        </div>
...
const props=withDefaults(defineProps<{record?:any;missions?:any[];auditorRole?:string;auditeurNom?:string;backUrl?:string;urlStore?:string;urlUpdate?:string|null;urlSoumettre?:string|null;urlValider?:string|null;urlIa?:string|null;missionContext?:any;}>(),{record:null,missions:()=>[],auditorRole:'AJ',auditeurNom:'',backUrl:'/',urlStore:'',urlUpdate:null,urlSoumettre:null,urlValider:null,urlIa:null,missionContext:()=>({})})
const form=reactive<any>({id:null,code:'',statut:'draft',validation_note:'',intitule:'',processus:'',version:'V1',date:'',description_narrative:'',synthese_validations:'',...(props.record??{})})
const raw=props.record?.activites_json;const activites=reactive<any[]>(raw?(typeof raw==='string'?JSON.parse(raw):raw):[])
const processing=ref(false)
const isoSymbols=[{n:'Traitement',i:'□',c:'#6d28d9'},{n:'Décision',i:'◇',c:'#1e40af'},{n:'Document',i:'⌐',c:'#065f46'},{n:'Saisie',i:'⌂',c:'#b45309'},{n:'Données',i:'⊞',c:'#be185d'},{n:'Renvoi',i:'○',c:'#0f172a'},{n:'Op. manuelle',i:'▽',c:'#0891b2'},{n:'Flux',i:'→',c:'#7c3aed'}]
function buildPayload(){return{mission_id:props.missionContext?.mission_id,assignment_id:props.missionContext?.assignment_id,procedure_code:props.missionContext?.procedure_code,test_ref:props.missionContext?.test_ref,intitule:form.intitule,processus:form.processus,version:form.version,date:form.date,description_narrative:form.description_narrative,synthese_validations:form.synthese_validations,activites_json:activites}}
